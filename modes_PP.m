...
% fetch data
data_folder='/home/adrian/Git/GitHub/work/param-infer-clicks/data/';
p_NLL=load([data_folder,'joint_PP_ntrials_99999_noise_1.mat']);
m500=load([data_folder,'mse_nonlin_fig4_iteration2_',num2str(500),...
    'trials.mat']);
m100=load([data_folder,'mse_nonlin_fig4_iteration2_',num2str(100),...
    'trials.mat']);

% check number of modes falling outside function range
fprintf('NLL 500 %d modes > 2.5\n',sum(m500.modes_nonlinlin>2.5))
fprintf('NLL 100 %d modes > 2.5\n',sum(m100.modes_nonlinlin>2.5))

% map modes to PP values
pp_NLL_500=mapmode2pp(m500.modes_nonlinlin(m500.modes_nonlinlin<=2.5),...
    p_NLL.hs, p_NLL.PP(:,68));
pp_NLL_100=mapmode2pp(m100.modes_nonlinlin(m100.modes_nonlinlin<=2.5),...
    p_NLL.hs, p_NLL.PP(:,68));


% whisker plots with upper bound
figure()
n=68; nt=[100,500];
for ii=1:2
    ax=subplot(1,2,ii);
...
lw=2; % linewidth
data_folder='/home/adrian/Git/GitHub/work/param-infer-clicks/data/';
p_LNL=load([data_folder,'joint_PP_ntrials_99999_noise_1.mat']);
m500=load([data_folder,'mse_lin_fig4_iteration2_',num2str(500),'trials.mat']);
m100=load([data_folder,'mse_lin_fig4_iteration2_',num2str(100),'trials.mat']);
...
%load([data_folder,'joint_PP_LL_ntrials_99999_noise_1.mat'])
%load([data_folder,'joint_PP_LL_ntrials_10000_noise_1.mat'])
m500=load([data_folder,'mse_lin_fig4_iteration2_',num2str(500),'trials.mat']);
m100=load([data_folder,'mse_lin_fig4_iteration2_',num2str(100),'trials.mat']);
...
gs=thetas_1;
pp_NLNL_500=mapmode2pp(m500.modes_linlin(m500.modes_linlin<=10), gs, PP(68,:));
pp_NLNL_100=mapmode2pp(m100.modes_linlin(m100.modes_linlin<=10), gs, PP(68,:));
...
%load([data_folder,'joint_PP_LL_ntrials_99999_noise_1.mat'])
%load([data_folder,'joint_PP_LL_ntrials_10000_noise_1.mat'])
m500=load([data_folder,'mse_nonlin_fig4_iteration2_',num2str(500),'trials.mat']);
m100=load([data_folder,'mse_nonlin_fig4_iteration2_',num2str(100),'trials.mat']);
