function pp=predictive_power_onTheFly(target_model_type, target_model_params,...
...
db_params=fetch_params(dbname,ref_model_type);

% compute target and reference models' choices
num_trials=diff(trial_range)+1;
num_disc=length(target_model_params.disc);
model_choices=zeros(num_disc,num_trials);
ref_model_choices=zeros(1,num_trials);
...
for tr=1:num_trials
    ...
    if strcmp(target_model_type,'lin')
        ...
        % conditional block to compute reference decision
        if strcmp(ref_model_type,'lin')
            ref_model_choices(tr)=decide_lin_noise(left_train, right_train,...
                db_params.disc, db_params.kappa, db_params.noise, 0);
        elseif strcmp(ref_model_type,'nonlin')
            num_clicks=length(left_train)+length(right_train);
            noise_matrix=normrnd(db_params.kappa,db_params.noise,...
                [num_clicks,1]);
            ref_model_choices(tr)=decide_nonlin_noise(db_params.T,left_train,...
                right_train, db_params.disc, 0, noise_matrix);
        end
    elseif strcmp(target_model_type,'nonlin')
        ...
        % conditional block to compute reference model choices
        if strcmp(ref_model_type,'lin')
            ref_model_choices(tr)=decide_lin_noise(left_train, right_train,...
                db_params.disc, db_params.kappa, db_params.noise, 0);
        elseif strcmp(ref_model_type,'nonlin')
